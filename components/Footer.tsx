import { FaLocationArrow } from "react-icons/fa6";

import { socialMedia } from "@/data";
import { MagicButton } from "./ui/MagicButton";

const Footer = () => {
  return (
    <footer className="w-full pt-20 pb-10 mt-20 mb-20 md:mb-0" id="contact">
      {/* background grid
      <div className="w-full absolute left-0 -bottom-72 min-h-96">
        <img
          src="/footer-grid.svg"
          alt="grid"
          className="w-full h-full opacity-50 "
        />
      </div> */}

      <div className="flex flex-col items-center max-w-7xl mx-auto px-10">
        <h1 className="font-bold text-4xl md:text-5xl text-center lg:max-w-[45vw]">
          Ready to take <span className="text-purple">your</span> digital
          presence to the next level?
        </h1>
        <p className="text-white-200 md:mt-10 my-5 text-center">
          Reach out to me today and let&apos;s discuss how I can help you
          achieve your goals.
        </p>
        <a href="mailto:peteremad132oo5@gmail.com" target="_blank">
          <MagicButton
            title="Let's get in touch"
            icon={<FaLocationArrow />}
            position="right"
          />
        </a>
      </div>
      <div className="flex mt-16 md:flex-row flex-col justify-between items-center max-w-7xl mx-auto px-10 w-full gap-5">
        <p className="md:text-base text-sm md:font-normal font-light whitespace-nowrap">
          Copyright © 2026 Peter Emad
        </p>

        <div className="flex items-center md:gap-3 gap-6">
          {socialMedia.map((info) => (
            <a
              key={info.id}
              href={info.link}
              target="_blank"
              rel="noopener noreferrer"
              title={info.link}
              className="w-10 h-10 flex justify-center items-center rounded-lg border border-black-300 bg-black-200 hover:scale-110 transition"
            >
              <img
                src={info.img}
                alt="icons"
                width={20}
                height={20}
                className={info.invert ? "invert" : ""}
              />
            </a>
          ))}
        </div>
      </div>
    </footer>
  );
};

export default Footer;
